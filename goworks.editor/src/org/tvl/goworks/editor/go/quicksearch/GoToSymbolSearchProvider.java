...
public class GoToSymbolSearchProvider implements SearchProvider {
    ...
    public void evaluate(SearchRequest request, SearchResponse response) {
        ...
        for (GoProject project : projects) {
            ...
            for (PackageModel packageModel : projectCache.getPackages()) {
                for (CodeElementModel codeElementModel : packageModel.getMembers()) {
                    if (codeElementModel instanceof TypeModel || codeElementModel instanceof PackageModel) {
                        // these are handled by GoToTypeSearchProvider
                        continue;
                    }

                    if (!examineModel(request, response, codeElementModel)) {
                        return;
                    }
                }
            }
